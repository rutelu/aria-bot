...
const BENI_SEED = {
  ...
  campaignVersion: 'cochabamba-2026-09-26c-40y50',   // 25 sep: vuelve Cochabamba por un día
  prevaloraciones: true, // esta campaña incluye pre-valoraciones de cirugías con el especialista presente
  promo: 'Valoración GRATIS (sin costo) y 40 por ciento de descuento en CUALQUIER tratamiento solo por agendar, SIN ninguna condición (no hace falta traer a nadie). Y si además TRAE a alguien recomendado que se atienda, el descuento sube a 50 por ciento. Es una promoción relámpago de un solo día: sábado 26 de septiembre.',
...
En las campañas/jornadas ACTIVAS suele haber descuentos especiales (en esta jornada relámpago: 40% en cualquier tratamiento sin condiciones, 50% si trae a alguien que se atienda); por el momento NO se pide depósito para reservar: la reserva queda registrada y se envía un recordatorio para confirmar. Fuera de campaña aplican los precios de referencia y la valoración normal.
...
RESERVA HECHA (lo resolvés VOS, NO escales): si el cliente dice que ya reservó/agendó, o llega desde el sitio web después de reservar, respondé vos misma con calidez. ⚠️ IMPORTANTE: ESTA JORNADA NO TIENE PAGO NI DEPÓSITO — la reserva YA queda confirmada y la valoración es GRATIS. Por eso NUNCA hables de "pago", "comprobante", ni de que "le llegará algo por correo". Solo FELICITALO, confirmale su cita, y recordale con entusiasmo que CON SU RESERVA YA GANÓ el 40% de descuento en cualquier tratamiento sin condiciones, y que si trae a un recomendado que se atienda obtiene 50% OFF (pásale el link para compartir: harmonieinstitute.com/jornada). NO avises a un humano por esto. Ejemplo: "¡Felicidades por tu reserva! 🎉 Tu cita ya quedó confirmada 💛 Y con tu reserva ganaste el 40% de descuento en cualquier tratamiento, sin condiciones; si traes a un recomendado que se atienda, ¡obtienes 50% OFF! Compartí la jornada aquí 👉 harmonieinstitute.com/jornada. ¡Te esperamos!".
...
function buildBeniSection(cfg, dispo, _rsvPropia) {
  ...
  s += '👋 UN SALUDO SUELTO NO SE CONTESTA CON UNA PREGUNTA VACÍA (REGLA DURA): cuando alguien con quien YA VENÍAS HABLANDO escribe algo corto y sin contenido ("hola", "buenas tardes", "hoy", "disculpe"), ⛔ NO respondas "¿en qué te ayudo?" ni "aquí sigo, ¿qué necesitas?". Esa respuesta la obliga a explicarte todo otra vez y se siente como si la hubieras olvidado. LEE EL HILO y retoma lo concreto que quedó pendiente: (a) si YA TIENE UNA CITA, salúdala por su nombre y menciónasela ("¡Buenas noches, Nilza! 💛 ¿Todo listo para tu cita del martes a las 20:00?"); (b) si estaban AGENDANDO y quedó a medias, retoma justo ahí ("¡Hola Rosa! Quedamos en el miércoles a las 14:00, ¿te la confirmo?"); (c) si preguntó por un TRATAMIENTO y no siguió, retómalo ("¡Hola! ¿Seguimos con lo del botox? La valoración es gratis y el miércoles tengo horas libres"). Solo puedes preguntar "¿en qué te ayudo?" cuando de verdad NO haya nada anterior en el hilo, es decir en el primer mensaje de una conversación nueva.\n\n';
  ...
  s += '⛔⛔ CÓMO TERMINA TU PRIMER MENSAJE (REGLA LITERAL, si la incumples se pierde la conversación): están PROHIBIDAS estas frases para cerrar — "¿alguno te llama la atención?", "¿hay alguno que te llame la atención?", "¿cuál te interesa?", "¿alguno te interesa?", "¿qué tratamiento te gustaría?" —. Todas le piden ELEGIR a alguien que recién está averiguando, y por eso no contesta (2/09: las tres personas del Beni se fueron justo ahí). Tu primer mensaje TERMINA SIEMPRE ofreciendo la valoración GRATIS con un día concreto y una pregunta de sí o no. Copia esta forma: "La valoración con el especialista es GRATIS y ahí te dice exactamente qué te conviene. [DÍA CONCRETO de la lista de arriba] estamos en [SEDE], ¿te aparto una hora?". Si ELLA nombra después un tratamiento, ahí sí le das su precio y vuelves a cerrar pidiéndole la reserva.\n\n';
...
async function toolConsultarDisponibilidad(args, cfg) {
  ...
  for (const d of dias) {
    ...
    if (enGracia.length) {
      ...
      entry.frase_para_decir = 'Esa hora ya pasó hace un ratito, pero si venís ahora mismo todavía te alcanzamos a atender 💛';
...
function _fmtSalida(texto, canal) {
  ...
  // Por WhatsApp, dar NUESTRO número es absurdo: la persona ya está escribiendo ahí.
  // Valeria lo hacía al derivar tratamientos fuera de la jornada ("el equipo te atiende
  // por WhatsApp: +591 76951552"), y encima hablaba del equipo en tercera persona.
  if (canal === 'wa') {
    t = t.replace(/(?:por |al |escr[ií]be(?:nos|le)s? a |cont[aá]ctal[oa]s? a |llama(?:l[oa]s)? a )?\+?\s?591[\s-]?7\s?6[\s-]?9\s?5\s?1[\s-]?5\s?5\s?2/gi, 'por aquí mismo');
    t = t.replace(/(?:por |al )?\b76[\s-]?951[\s-]?552\b/gi, 'por aquí mismo');
...
app.post('/chat', async (req, res) => {
  ...
  try {
    ...
    const webNote = '\n\n---\n[CANAL: CHAT WEB de harmonieinstitute.com — REGLAS DE ESTE CANAL (PRIORIDAD MÁXIMA, por encima de todo lo de arriba):\n'
      ...
      + '7) NUNCA INVENTES UNA CAMPAÑA VIGENTE (REGLA DURA): si la sección "JORNADA ACTIVA" de arriba dice que NO hay ninguna activa, está PROHIBIDO decir "nuestra jornada actual", "la campaña de ahora" o cualquier frase que dé a entender que hay una en curso, y está PROHIBIDO prometer sus beneficios (40%, 50% por recomendado, valoración gratis) como si se pudieran reclamar hoy. Si preguntan por esos descuentos y NO hay jornada activa, explica con calidez que son beneficios de nuestras jornadas, que en este momento no hay una en curso, y ofrece agendar su valoración o dejar sus datos para avisarle de la próxima. Nunca le prometas a alguien un descuento que hoy no puede reclamar.\n'
...
app.get('/debug/fichas', async (req, res) => {
  if (req.query.key !== 'diag-9x') return res.status(403).json({ error: 'no' });
  if (!db) return res.json({ error: 'sin base' });
  const q = String(req.query.buscar || '').toLowerCase().trim();
  try {
    const snap = await db.collection('fichas').get();
    const out = [];
    snap.forEach(function (d) {
      const x = d.data() || {};
      const nom = String(x.nombre || x.patientName || '');
      if (q && nom.toLowerCase().indexOf(q) === -1 && String(d.id).indexOf(q) === -1) return;
      out.push({ tel: d.id, nombre: nom, actualizada: x.actualizadoAt ? new Date(x.actualizadoAt._seconds * 1000).toISOString() : null,
                 tieneFicha: !!(x.ficha && Object.keys(x.ficha).length), visitas: (x.reservas || []).length });
    });
    out.sort(function (a, b) { return String(b.actualizada).localeCompare(String(a.actualizada)); });
    res.json({ total: snap.size, encontradas: out.length, fichas: out.slice(0, 40) });
  } catch (e) { res.json({ error: e.message }); }
});
...
app.get('/debug/reenganche', async (req, res) => {
  ...
  for (const e of elegidos) {
    ...
    const texto = saludo + ' Te escribo de Harmonie. Para ' + _lbl.toLowerCase() + ' todavía me quedan horas libres en la jornada de ' + (dia.subsede || '') + ': ' + tres + '. La valoración es gratis y con tu reserva ya ganas el 40% de descuento en cualquier tratamiento. ¿Te agendo una?';
    e.mensaje = texto;
    if (!enviar) continue;
